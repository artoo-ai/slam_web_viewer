...
| `scan_main` | ≤10 Hz | **bin** — same packing as `scan` (float32 LE `[x,y,z,intensity] × N`, map frame). The MAIN nav/SLAM band: the 2D `/scan` (slam_bringup `pointcloud_to_laserscan`) that feeds slam_toolbox and the costmap's main `obstacle_layer`. Forwarded so the viewer can show which cloud points the planner/SLAM actually use. Advertised only when the main `scan` channel is a 3D cloud (else `/scan` *is* `scan`); toggle off with `--no-scan2d-points`. Droppable channel. |
